...
function Login() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col">
      {/* TOPO COM IMAGENS */}
      <div className="h-[45vh] bg-white relative flex items-center justify-center overflow-hidden">
        <img
        ...
        <img
          ...
          className="absolute top-36 left-0 animate-float-fast"
        />
        <img
          src={homifazendocompra}
          alt="homem fazendo compra"
          className="w-auto h-[80%] object-contain animate-zoom-in"
        />
      </div>

      {/* FORMULÁRIO */}
      <div className="flex-1 bg-[#F9A01B] flex flex-col items-center justify-center px-4">
        <h1 className="text-[40px] font-semibold text-center font-[Poppins] text-black animate-slide-up">
          Bem vindo de volta!
        </h1>

        <div className="flex flex-col gap-4 w-full max-w-[821px] mt-6 animate-fade-in">
          <Input
            type="email"
            placeholder="Email"
            value={email}
          ...
          <div className="relative w-full">
            ...
            <button
              ...
              className="absolute right-6 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-[#25992E] transition-colors"
            >
              {showsenha ? (
          ...
          {errorMsg && (
            <p className="text-red-500 text-[20px] mt-2 text-center font-semibold animate-shake">
              {errorMsg}
            </p>
          )}
        </div>
        ...
        <div className="flex justify-center w-full mt-8">
          <Button
            onClick={handleLogin}
            disabled={loading}
            className="relative w-[410px] h-[92px] bg-[#25992E] rounded-[66px] text-white text-[36px] font-[Poppins] font-bold 
        ...
        <div
          className="flex flex-row items-center justify-center mt-6 space-x-2 cursor-pointer hover:scale-105 transition-transform duration-300"
          onClick={() => navigate("/cadastro")}
        >
          <p className="text-[24px] text-center font-[Poppins] text-black">
            Não tem uma conta?
          </p>
          <p className="text-[24px] text-[#25992E] text-center font-[Poppins] font-semibold hover:underline">
            Cadastre-se aqui
          </p>
        </div>
      </div>
